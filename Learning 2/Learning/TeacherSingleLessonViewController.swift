...
class TeacherSingleLessonViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell.StdName.text = self.studentName[indexPath.row] + " " + self.studentID[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "StudentSloutions") {
            let note = segue.destination as! StudentHWExerciesViewController;
           
            note.student.id = self.studentID[selected]
            note.hwID = self.hwID
